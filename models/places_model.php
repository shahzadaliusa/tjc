...
class Places_Model extends Model{
    ...
    function indexFetch(){
        $sth = $this->db->prepare("SELECT * FROM in_places");
        $sth->execute();
    ...
    function pkgFetch($arg1,$arg2){
      $sth = $this->db->prepare("SELECT * FROM in_packages WHERE in_pkg_zone='$arg1' AND in_pkg_place='$arg2'");
        $sth->execute();
        $sth->setFetchMode(PDO::FETCH_ASSOC);
        $result = $sth->fetchAll();
        return $result;  
